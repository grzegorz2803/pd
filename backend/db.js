...
async function updateOrInsertPoints(cardID, points, parID) {
  ...
  const year = now.getFullYear();
  const month = String(now.getMonth() + 1).padStart(2, "0");
  const tables = [`${parID}_${year}`, `${parID}_${month}_${year}`];
  try {
...
async function sendMessage(cardId, subject, message, res) {
  try {
    if (!subject || !message) {
      return res
        .status(400)
        .json({ message: "Temat i treść wiadomości są wymagane." });
    }
    ...
    if (result.affectedRows === 1) {
      ...
      return res
        .status(500)
        .json({ message: "Nie udało się zapisać wiadomości." });
    }
  } catch (error) {
    console.error("Błąd przy zapisie wiadomości:", error);
    return res
      .status(500)
      .json({ message: "Błąd serwera podczas wysyłania wiadomości." });
  }
}
...
async function saveScheduleToDB(dateFrom, dateTo, scheduleArray) {
  ...
  for (const entry of scheduleArray) {
    ...
    for (const card_id of user_ids) {
      valuesToInsert.push([dateFrom, dateTo, day_of_week, time, card_id]);
    }
  }
