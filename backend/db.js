...
async function saveMeatingResults(cardId,results, res){
    ...
    `;
       ...
       VALUES (?, 0, ?)`,
        ...
        return res.status(500).json({ message: "Błąd serwera przy zapisywaniu zbiórki " });
    }
...
async function sendModeratorMessage(senderCardId, subject, body, recipientId = null) {
    try {
        const isReply = 0;
        const replyTo = null;
...
async function replayToMessage(moderatorCardId, replyToId, recipientCardId, body)  {
    try {
        await pool.execute(
            `INSERT INTO messages 
       ...
       VALUES (?, ?, NULL, ?, 1, ?, NOW())`,
        ...
        await pool.execute(`UPDATE messages SET hidden_for_moderator = 1 WHERE id= ?`,[replyToId]);
    } catch (err) {
